...
public class ThreadPoolRevisted {
	...
	public static void demoThreadsCreatedByThreadPool() throws InterruptedException, ExecutionException {
		...
		// submit 10 similar tasks and watch that they are executed from different threads
		// unlike Thread class, available threads are reused here
		for (int i = 0; i < 10; i++) {
	...
	public static void demoThreadFactory() {
		...
		// Creates a thread pool that creates new threads as needed using the provided ThreadFactory, but will reuse previously constructed threads when they are available.
	...
	public static void demoDifferentExecutorServices() {
		...
		ExecutorService pool = Executors.newCachedThreadPool();
		// Try using these thread pools an how it influences the threads where the tasks are executed
